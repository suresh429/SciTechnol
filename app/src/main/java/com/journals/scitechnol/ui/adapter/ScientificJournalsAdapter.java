...
import static com.journals.scitechnol.network.RetrofitService.IMAGE_HOME_URL;
...
public class ScientificJournalsAdapter extends RecyclerView.Adapter<ScientificJournalsAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ScientificJournalsAdapter.ViewHolder holder, int position) {
        ...
        holder.rowItemBinding.txtJournalDescription.setText(Html.fromHtml(modelList.get(position).getJournalCatDesc()));

        Glide.with(context)
                .load(IMAGE_HOME_URL +modelList.get(position).getJournalImage())
        ...
        holder.rowItemBinding.getRoot().setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                bundle.putString("catId", modelList.get(position).getJournalCat());
